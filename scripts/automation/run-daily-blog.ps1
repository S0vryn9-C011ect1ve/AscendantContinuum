...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Log "✅ Blog post completed successfully"
        ...
        if ($gitStatus) {
            ...
            Set-Location $projectRoot
            Write-Log "✅ Deployed to Firebase"
        } else {
            Write-Log "No blog changes to commit"
        }
    } else {
        Write-Log "❌ Blog post failed with exit code: $LASTEXITCODE"
    }
} catch {
    Write-Log "❌ Error running blog poster: $_"
}
