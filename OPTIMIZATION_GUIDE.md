# ⚡ Optimization Guide - 60fps on Low-End Devices
**Make Your Game Run Smoothly Everywhere**

## Performance Targets

**Minimum Specs (Target):**
- Device: 2019 mid-range phone (Galaxy S8, iPhone 8)
- CPU: Snapdragon 835 or Apple A11
- RAM: 4GB
...
**Don't optimize blindly!** Use Unity Profiler to find actual problems.

### How to Profile

1. **Open Profiler:**
   - Window → Analysis → Profiler
   - Or Ctrl+7 (Cmd+7 on Mac)

2. **Run game in Play Mode**
3. **Click Record** (red circle)
4. **Play for 30-60 seconds** performing various actions
5. **Stop recording**
6. **Analyze graph:**
   - CPU: Time spent on scripts, rendering, physics
   - GPU: Time spent rendering on graphics card
   - Memory: RAM usage
   - Rendering: Draw calls, batches, triangles

### What to Look For

**CPU Bottleneck (CPU bar tall):**
- Too much scripting logic
- Too many Update() calls
- Complex calculations every frame

**GPU Bottleneck (Rendering bar tall):**
- Too many draw calls
- Complex shaders
...
- Spikes = garbage collection (GC)
- Too high overall = will crash on low-RAM devices

---

## 🔧 OPTIMIZATION TECHNIQUES

### 1. REDUCE DRAW CALLS (GPU)

**Problem:** Each sprite/mesh is a separate draw call. Target: <100 draw calls

**Solutions:**

**A. Sprite Atlas (Highly Recommended)**
1. Select all sprites for one realm (e.g., all Emberforge sprites)
2. Right-click → Create → 2D → Sprite Atlas
3. Name: "EmberforgeAtlas"
4. Add sprites to atlas (drag into Objects for Packing)
5. Unity batches these into 1-2 draw calls automatically

**Before:** 50 sparks = 50 draw calls  
**After:** 50 sparks = 1-2 draw calls ✅

**B. Reduce Active GameObjects**
- Only spawn objects visible on screen
- Destroy objects that drift off-screen
- Use object pooling (already implemented in your scripts!)

---

### 2. OPTIMIZE PARTICLES (GPU)

**Problem:** Too many particles = GPU overload

**Solutions:**

**A. Reduce Particle Count**
```csharp
// In ParticleManager.cs or particle system
ParticleSystem ps = GetComponent<ParticleSystem>();
var emission = ps.emission;
emission.rateOverTime = 10; // Was 50, now 10 (80% reduction)
```

**B. Use Simpler Particle Shaders**
- Default Particle shader is ok
- Avoid complex shaders with multiple textures

**C. Limit Active Particle Systems**
- Max 5 particle systems active simultaneously
- Reuse/pool particle systems

**D. Reduced Motion Mode Simplification**
```csharp
if (AccessibilityManager.Instance.reducedMotion)
{
    // Disable particles entirely
    particleSystem.Stop();
    
    // Or show simple static sprite instead
    simpleEffectSprite.SetActive(true);
}
```

---

### 3. OPTIMIZE SCRIPTS (CPU)

**Problem:** Too much code running every frame

**Solutions:**

**A. Reduce Update() Calls**

**Bad:**
```csharp
void Update()
{
    // Runs 60 times per second = expensive
    CheckForInput();
    UpdateUI();
    CheckCollisions();
}
...
void Update()
{
    // Only check input in Update (frame-dependent)
    CheckForInput();
}

void FixedUpdate()
{
    // Physics checks here (50 times per second)
    CheckCollisions();
}
...
void UpdateUI()
{
    // Call only when needed (e.g., OnSparkCollected)
}
```

**B. Cache Component References**
...
void Update()
{
    sr.color = Color.red; // Fast!
}
```

**C. Use Object Pooling** (Already Implemented!)
Your code already has this! Verify it's enabled:
- ParticleManager.cs pools 30 effects
- AudioManager.cs pools 20 sources
- EmberforgeSparks.cs pools 50 sparks

**D. Avoid GameObject.Find()**
Never call in Update()! Cache references in Start().

---

### 4. OPTIMIZE TEXTURES (Memory + GPU)

**Problem:** Large textures use too much memory

**Solutions:**

**A. Compress Textures**
1. Select texture in Project window
2. Inspector → Platform Settings (Android/iOS icon)
3. Format:
   - **Android:** ASTC 6x6 (good quality) or 8x8 (smaller)
   - **iOS:** ASTC 6x6 or PVRTC 4-bit
   - **WebGL:** DXT5 or Automatic
4. Max Size:
   - Backgrounds: 2048x2048 (or 1024x1024 if looks ok)
   - UI elements: 512x512
   - Small sprites: 256x256 or 128x128
5. Check "Generate Mip Maps" for 3D (unchecked for 2D usually)

**Impact:** Can reduce build size by 50-70%!

**B. Reduce Texture Resolution**
If your background is 4096x4096:
- Resize to 2048x2048 in GIMP (free)
- Quality loss is minimal on mobile screens

---

### 5. OPTIMIZE AUDIO (Memory)
